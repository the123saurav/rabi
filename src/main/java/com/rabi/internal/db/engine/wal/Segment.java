    ...
    if (Files.exists(path)) {
      try (final FileChannel ch = FileChannel.open(path, StandardOpenOption.READ)) {
        ...
        while ((numRead = ch.read(loadBuffer)) > 0) {
          ...
          LOG.debug(
              String.format("last position: %d, totalRead: %d", loadBuffer.position(), totalRead));
          ...
          while (loadBuffer.position() < lastPos) {
            ...
            if (e == null) {
              loadBuffer.reset();
              LOG.debug(path.getFileName() + " - buffer underflow, position is: " +
                  loadBuffer.position() + " reading more data");
              break;
            }
